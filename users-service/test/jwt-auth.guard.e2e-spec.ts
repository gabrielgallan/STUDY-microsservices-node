...
describe('JwtAuthGuard', () => {
	...
	it('bypasses Passport when the handler is public', () => {
		const context = {
			getClass: () => PublicControllerFixture,
			getHandler: () => PublicControllerFixture.prototype.publicRoute,
		} as unknown as ExecutionContext

		expect(guard.canActivate(context)).toBe(true)
	})

	it('returns only an authenticated identity', () => {
		const user = {
